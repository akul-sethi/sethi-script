...
// Prints an instruction which takes an operand reprsenting a constant in the
// constant pool.
...
// Prints an instruction which takes an operand representing a constant on the
// vm stack (indexed from bottom).
...
// Prints instruction which takes two bytes for an operand which represents the
// jump length
...
// Prints instruction which takes one byte for an operand which reprsents number
// of params
...
// Prints instruction for table with a 1 byte operand representing number of
// fields
...
//     printValue(chunk->constants.values[index]);
//     printf("=");
//     printValue(*get(table,
//     (ObjString*)chunk->constants.values[index].as.obj)); return offset + 2;
// }
...
int dissasembleInstruction(Chunk *chunk, int offset) {
  ...
  case OP_FALSIFY:
    return simpleInstruction("OP_FALSIFY", offset);
  case OP_NIL:
  ...
  case OP_SET_GLOB:
    return constantInstruction("OP_SET_GLOB", chunk, offset);
  case OP_GET_GLOB:
    return constantInstruction("OP_GET_GLOB", chunk, offset);
  case OP_SET_LOC:
    return localInstruction("OP_SET_LOC", chunk, offset);
  case OP_GET_LOC:
    return localInstruction("OP_GET_LOC", chunk, offset);
  case OP_JUMP:
    return jumpInstruction("OP_JUMP", chunk, offset);
  case OP_JUMP_BACK:
    return jumpInstruction("OP_JUMP_BACK", chunk, offset);
  ...
  case OP_TABLE:
    return tableInstruction("OP_TABLE", chunk, offset);
  case OP_NAMESPACE:
